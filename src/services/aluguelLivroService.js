...
class AluguelLivroService {
  ...
  async calcularDataDevolucao(dataAlugado, diasAlugados) {
    const dataAlugadoObject = new Date(dataAlugado);
    const dataAlugadoMilisegundo = dataAlugadoObject.getTime();
    const diasAlugadosMilisegundo = diasAlugados * 24 * 60 * 60 * 1000;

    const dataRetornoMilisegundo =
      dataAlugadoMilisegundo + diasAlugadosMilisegundo;

    return new Date(dataRetornoMilisegundo);
  }
}
